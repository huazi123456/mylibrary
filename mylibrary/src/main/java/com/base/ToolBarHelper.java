...
/**
 * ToolBar帮助类 动态创建ToolBar
 * Created by zhengjiabinn 2016/7/14.
 * time : 10:45
 */
public class ToolBarHelper { /*上下文，创建view的时候需要用到*/
    ...
    * */
    ...
    public ToolBarHelper(Context context, View view, int toolBarResId) {
        this.mContext = context;
        mInflater = LayoutInflater.from(mContext); /*初始化整个内容*/
        initContentView(); /*初始化用户定义的布局*/
        initUserView(view); /*初始化toolbar*/
        initToolBar(toolBarResId);
    }
    ...
    private void initToolBar(int toolBarResId) { /*通过inflater获取toolbar的布局文件*/
    ...
    private void initUserView(int id) {
        ...
        int toolBarSize = DimensUtil.dpToPixels(mContext, 44);
        params.topMargin = toolBarSize;
        mContentView.addView(mUserView, params);
    }

    private void initUserView(View mUserView) {
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        int toolBarSize = DimensUtil.dpToPixels(mContext, 44);
        params.topMargin = toolBarSize;
        mContentView.addView(mUserView, params);
